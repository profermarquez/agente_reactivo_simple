...
#agente con memoria
class ReactiveMemoryAgent:
    def __init__(self, target_range):
        self.target_range = target_range
        self.last_action = None  # Memoria del último comando enviado

    def act(self, observation):
        temperature = observation[0]

        if temperature < self.target_range[0]:  
            action = 0  # Encender calefacción
        elif temperature > self.target_range[1]:  
            action = 1  # Apagar calefacción
        else:
            action = self.last_action if self.last_action is not None else 0  # Mantener acción anterior si está en el rango

        self.last_action = action  # Guardar la acción tomada
        return action
...
agent = ReactiveMemoryAgent(target_range=(180, 220))
